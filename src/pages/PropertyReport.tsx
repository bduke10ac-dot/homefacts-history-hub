...
export default function PropertyReport() {
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      pdf.save(`Orivaz-${type}-${property?.address_line?.replace(/\s+/g, "-") ?? "report"}.pdf`);
    } catch (e: any) {
        ...
        <div id="report-printable" className="space-y-6">
          ...
          />
              ...
              <p className="text-center text-xs text-muted-foreground">
                Generated by Orivaz AI{report ? ` on ${format(new Date(report.created_at), "MMM d, yyyy 'at' h:mm a")}` : ""} · AI-generated; verify with a licensed professional.
              </p>
            </>
          )}
        </div>
