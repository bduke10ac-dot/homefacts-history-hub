...
export function PropertyView({ shared = false }: { shared?: boolean }) {
  ...
  const downloadPdf = async () => {
    ...
    pdf.save(`Orivaz-${property?.address_line.replace(/\s+/g, "-")}.pdf`);
  };
        ...
        <div id="report-printable" className="space-y-6 bg-background">
          <div className="overflow-hidden rounded-2xl border bg-card shadow-card">
            <div className="bg-gradient-hero px-8 py-10 text-primary-foreground">
              <div className="flex items-center gap-2 text-xs uppercase tracking-wider opacity-80"><ShieldCheck className="h-4 w-4" />Orivaz Property Report</div>
          ...
          <p className="text-center text-xs text-muted-foreground">Generated by Orivaz Property Report on {format(new Date(), "MMM d, yyyy")}</p>
        </div>
      </div>
    </div>
  );
