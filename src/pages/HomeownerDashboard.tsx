...
import { Home, MapPin, Plus, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-10">
        ...
        <Section title="Claimed properties" empty="You haven't claimed a home yet. Search for your address to get started." items={claimed} counts={counts} accentBadge="Owner" />
...
function Section({ title, empty, items, counts, accentBadge }: { title: string; empty: string; items: Property[]; counts: Record<string, RecordCount>; accentBadge?: string }) {
      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {items.map((p) => {
            ...
            return (
              <Link key={p.id} to={`/property/${p.id}`} className="rounded-xl border bg-card p-5 shadow-card transition-all hover:border-primary/40 hover:shadow-elevated">
